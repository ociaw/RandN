...
namespace RandN.Rngs
{
    public sealed class ChaChaIntrinsicsTests
    {
        ...
        {
            var rng = ChaChaIntrinsics.Create(new UInt32[] { 0, 0, 1, 0, 2, 0, 3, 0 }, 0, 0, 10);
        ...
        {
            var rng = ChaChaIntrinsics.Create(new UInt32[] { 0, 0, 1, 0, 2, 0, 3, 0 }, UInt64.MaxValue, 0, 10);
            Span<UInt32> buffer = stackalloc UInt32[64];
            rng.Generate(buffer);
            Assert.Equal(137206642u, buffer[0]);
            Assert.Equal(0ul, rng.Stream);
        ...
        {
            var rng = ChaChaIntrinsics.Create(new UInt32[] { 0, 0, 1, 0, 2, 0, 3, 0 }, 0, 0, 6);
            Span<UInt32> buffer = stackalloc UInt32[64];
            rng.Regenerate(buffer);
        ...
        {
            var rng = ChaChaIntrinsics.Create(new UInt32[] { 0, 0, 1, 0, 2, 0, 3, 0 }, UInt64.MaxValue, 0, 6);
            Span<UInt32> buffer = stackalloc UInt32[64];
            rng.Generate(buffer);
            Assert.Equal(1488489079u, buffer[0]);
            Assert.Equal(0ul, rng.Stream);
        ...
        {
            var rng = ChaChaIntrinsics.Create(new UInt32[] { 0, 0, 1, 0, 2, 0, 3, 0 }, 0, 0, 4);
            Span<UInt32> buffer = stackalloc UInt32[64];
            rng.Regenerate(buffer);
